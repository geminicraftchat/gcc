...
public class ChatManager {
    ...
    public void handleChat(Player player, String message) {
        ...
        if (isOnCooldown(playerId)) {
            ...
            player.sendMessage(Component.text("请等待 " + (remainingTime / 1000) + " 秒后再次发送消息")
                    .color(net.kyori.adventure.text.format.NamedTextColor.RED));
            return;
        }

        // 发送思考消息
        player.sendMessage(LegacyComponentSerializer.legacySection().deserialize(configManager.getThinkingFormat()));
    ...
    private void broadcastQuestion(Player player, String message) {
        ...
        String broadcastText = format
            .replace("{player}", player.getName())
            .replace("{message}", message);
        Component broadcast = LegacyComponentSerializer.legacySection().deserialize(broadcastText);
        ...
        if (configManager.getConfig().getBoolean("chat.broadcast.to_console", true)) {
            plugin.getComponentLogger().info(broadcast);
        }
    }
